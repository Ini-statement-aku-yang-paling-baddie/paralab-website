...
describe("FormulaScreeningCard", () => {
  ...
  it("menampilkan rule_id, versi, sumber, dan alasan yang bisa diaudit", () => {
    render(<FormulaScreeningCard screening={SCREENING} loading={false} error={null} />);
    expect(screen.getByText(/COMPAT-001/)).toBeTruthy();
    expect(screen.getByText(/rule v4 · source SRC-1/)).toBeTruthy();
    expect(screen.getByText(/asam askorbat berpotensi tidak stabil/)).toBeTruthy();
  });

  it("memperlakukan bahan unknown sebagai penghenti", () => {
    render(
      <FormulaScreeningCard
        screening={{ ...SCREENING, overall_status: "unknown" }}
        loading={false}
        error={null}
      />,
    );
    expect(screen.getByText(/screening dihentikan/i)).toBeTruthy();
  });
