...
import { Card, CardTitle, Pill, Stat, StatStrip, inputClass } from "@/components/paralab/ui";
  ...
  head: () => ({
    meta: [
      { title: "Gudang Bahan Baku | paralab.ai" },
      {
        name: "description",
        content:
          "Ketersediaan bahan baku laboratorium lengkap dengan asal bahan, nomor lot, kuantitas, nomor rak, dan tanggal kedaluwarsa.",
      },
      { property: "og:title", content: "Gudang Bahan Baku | paralab.ai" },
      {
        property: "og:description",
        content: "Pantau stok, lot, dan posisi rak bahan baku sebelum memulai batch penelitian.",
      },
...
function GudangPage() {
  ...
  const golonganOpsi = useMemo(
    () => ["Semua golongan", ...new Set(BAHAN_LIBRARY.map((b) => b.golongan))],
    [],
  );
  ...
  const daftar = useMemo(() => {
    ...
    return BAHAN_LIBRARY.filter((b) => {
      ...
      const cocokCari =
        q.length === 0 || (b.name + " " + b.inci + " " + b.cas).toLowerCase().includes(q);
  ...
  return (
    <AppShell
      judul="Gudang bahan baku"
      deskripsi="Ketersediaan bahan, asal bahan, nomor lot, kuantitas, dan posisi rak di gudang laboratorium."
    >
      <StatStrip>
        <Stat
          label="Jenis bahan"
          value={String(BAHAN_LIBRARY.length)}
          hint="Terdaftar pada katalog bahan"
        />
        <Stat label="Lot aktif" value={String(totalLot)} hint="Seluruh zona penyimpanan" />
        <Stat
          label="Lot karantina"
          value={String(karantina)}
          hint="Menunggu hasil pemeriksaan mutu"
        />
      ...
      <Card className="mt-5">
        <CardTitle
          title="Daftar ketersediaan bahan"
          sub="Tekan satu bahan untuk melihat lot, rak, asal bahan, dan struktur 2D molekulnya."
        />
        <div className="mb-4 flex flex-wrap gap-2">
          <div className="relative min-w-60 flex-1">
            ...
            <input
              ...
              placeholder="Cari nama bahan, INCI, atau nomor CAS"
              value={cari}
            ...
            <tbody>
              {daftar.map((b) => {
                ...
                return (
                  <tr
                    key={b.id}
                    className="cursor-pointer border-b border-border/70 hover:bg-secondary"
                    onClick={() => setDipilih(b.id)}
                  >
      ...
      <Card className="mt-5">
        <CardTitle
          title="Lot terbaru diterima"
          sub="Sepuluh penerimaan bahan terakhir beserta sertifikat analisisnya."
        />
        <ul className="space-y-2 text-sm">
          {[...SEMUA_LOT]
            ...
            .map((l) => (
              <li
                key={l.kode}
                className="flex flex-wrap items-center justify-between gap-2 border-b border-border/70 pb-2"
              >
                ...
                <span className="text-xs text-muted-foreground">
                  {l.kuantitas} {l.satuan} · rak {l.rak} · {l.asal} · diterima {l.diterima} ·{" "}
                  {l.coa}
                </span>
              </li>
            ))}
      ...
      <p className="mt-4 text-xs text-muted-foreground">
        Total {lotBahan(BAHAN_LIBRARY[0]!.id).length > 0 ? SEMUA_LOT.length : 0} lot tercatat pada
        sistem gudang simulasi.
      </p>
    </AppShell>
  );
}
