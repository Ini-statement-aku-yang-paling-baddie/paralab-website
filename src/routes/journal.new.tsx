  ...
  head: () => ({
    meta: [
      { title: "Tambah Jurnal Penelitian Baru | paralab.ai" },
      {
        name: "description",
        content:
          "Masukkan brief produk, dapatkan usulan formula AI lengkap dengan HPP, status halal, dan potensi clash, lalu ubah sesuai kebutuhan.",
      },
      { property: "og:title", content: "Tambah Jurnal Penelitian Baru | paralab.ai" },
      {
        property: "og:description",
        content:
          "Alur dari brief produk menjadi jurnal praktikum kosong yang siap diuji di laboratorium.",
      },
...
function JurnalBaruPage() {
  ...
  const mirip = useMemo(
    () => cariJurnalMirip(brief.judul, brief.kategori),
    [brief.judul, brief.kategori],
  );
  ...
  function simpanJurnal(bukaEditor = false) {
    ...
    const proyek: Project = {
      ...
      brief:
        brief.catatan ||
        "Penelitian " +
          brief.kategori.toLowerCase() +
          " untuk kulit " +
          brief.targetKulit.toLowerCase() +
          ".",
      targets,
      standar,
    ...
    navigate({
      to: bukaEditor ? "/doc/$id" : "/journal/$id",
      params: { id },
      search: { batch: 1 },
    });
  }
  ...
  return (
    <AppShell
      judul="Tambah Jurnal Baru"
      deskripsi="Dari brief produk sampai jurnal praktikum kosong yang siap diuji di laboratorium."
    >
        ...
        {LANGKAH.map((l, i) => (
          ...
          >
            <span className="flex size-5 items-center justify-center rounded-full bg-background text-xs">
              {i < langkah ? <Check className="size-3" /> : i + 1}
            </span>
            {l}
          </li>
        ))}
      ...
      {langkah === 0 && (
        <div className="grid gap-5 lg:grid-cols-3">
          <Card className="lg:col-span-2">
            <CardTitle
              title="Brief penelitian"
              sub="Isi kebutuhan produk yang ingin dikembangkan."
            />
            <div className="space-y-4">
              <Field label="Judul penelitian">
                <input
                  className={inputClass}
                  placeholder="Contoh: Face wash oil control untuk pria aktif"
                  value={brief.judul}
                  onChange={(e) => setBrief({ ...brief, judul: e.target.value })}
                />
              </Field>
              <div className="grid gap-4 sm:grid-cols-2">
                ...
                <Field label="Tipe kulit sasaran" hint="Pilihan menyesuaikan kategori produk">
                  <select
                    className={inputClass}
                    value={brief.targetKulit}
                    onChange={(e) => setBrief({ ...brief, targetKulit: e.target.value })}
                  >
                    {kulitOpsi.map((k) => (
                      <option key={k}>{k}</option>
                    ))}
              ...
              <Field
                label="Klaim yang diinginkan"
                hint="Klaim yang ditampilkan relevan dengan kategori yang dipilih"
              >
                    ...
                    return (
                      <button
                        ...
                        onClick={() =>
                          setBrief({
                            ...brief,
                            klaim: aktif ? brief.klaim.filter((x) => x !== k) : [...brief.klaim, k],
                          })
                        }
                        className={
              ...
              <Field
                label="Standar pengujian kosmetik"
                hint="Pilihan ini menentukan acuan metode pada template laporan praktikum"
              >
                ...
                <p className="mt-2 text-xs text-muted-foreground">
                  {standarTerpilih(standar).length} standar dipakai, antara lain{" "}
                  {standarTerpilih(standar)
                    .slice(0, 4)
                    .map((s) => s.kode)
                    .join(", ")}
                  .
              ...
              <Field
                ...
                hint="Segmen pasar, kendala produksi, atau permintaan khusus"
              >
                <textarea
                  className={inputClass + " min-h-24"}
                  value={brief.catatan}
                  onChange={(e) => setBrief({ ...brief, catatan: e.target.value })}
                />
              </Field>
          ...
          <Card>
            <CardTitle
              title="Penelitian serupa"
              sub="AI mencocokkan judul dengan arsip jurnal organisasi."
            />
            {brief.judul.trim().length < 4 && (
              <p className="text-sm text-muted-foreground">
                Ketik judul penelitian untuk melihat arsip yang relevan.
              </p>
            )}
                ...
                return (
                  <div key={proyek.id} className="rounded-xl border border-border p-3">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <Pill
                        variant={
                          proyek.status === "Sedang Berjalan" && skor > 70 ? "waspada" : "brand"
                        }
                      >
                        {skor}%
                      </Pill>
                    ...
                    {proyek.status === "Sedang Berjalan" && skor > 70 && (
                      <p className="mt-2 rounded-lg bg-warning-soft px-2.5 py-1.5 text-xs font-semibold text-warning">
                        Proyek ini masih berjalan dan sangat mirip, sebaiknya koordinasi dengan{" "}
                        {proyek.tim}.
                      </p>
                    )}
                    <button
        ...
        <div className="space-y-5">
            ...
            <Card className="xl:col-span-2">
              <CardTitle
                ...
                sub={
                  ...
                  " persen. Ubah kadar, tambah, atau hapus zat sesuai penilaian peneliti."
                }
                right={
                  <Pill variant={Math.abs(totalPersen - 100) < 0.5 ? "aman" : "waspada"}>
                    {Math.abs(totalPersen - 100) < 0.5 ? "Seimbang" : "Perlu penyesuaian"}
                  </Pill>
                }
              />
                      ...
                      <tr key={b.id} className="border-b border-border/70">
                        <td className="py-2.5 pr-3">
                          ...
                          <p className="text-[11px] text-muted-foreground">
                            CAS {b.cas} · {b.rumus} · pH kerja {b.phKerja[0]} sampai {b.phKerja[1]}{" "}
                            · lazim {b.dosis[0]} sampai {b.dosis[1]} persen
                          </p>
                          <StokPill
                            bahanId={b.id}
                            persen={b.percent}
                            onClick={() => setStokDibuka(b.id)}
                          />
                        </td>
            ...
            <div className="space-y-5">
              ...
              <Card>
                <CardTitle
                  ...
                  sub={
                    "Rule prototipe versi " +
                    kepatuhan.ruleVersion +
                    ", bukan persetujuan BPOM atau MUI"
                  }
                />
                <Pill
                  variant={
                ...
                <ul className="mt-3 space-y-2 text-xs text-muted-foreground">
                  ...
                  {kepatuhan.takDikenal.map((b) => (
                    <li key={b.id} className="text-warning">
                      {b.name} belum dapat dipetakan ke INCI atau CAS kanonis, tinjauan manusia
                      wajib.
                    </li>
                  ))}
                  {kepatuhan.status === "clear_for_current_screening" && (
                    <li>
                      Tidak ada rule prototipe yang aktif pada formula ini. Ini bukan pernyataan
                      halal, aman, atau lolos registrasi.
                    </li>
                  )}
                </ul>
              </Card>
            ...
            <div className="grid gap-5 lg:grid-cols-3">
              <Card>
                <CardTitle
                  ...
                  sub="Pemeriksaan sisi klien sebelum F2 dijalankan server"
                />
                <BandProvenance lapis="rule" />
                {clashes.length === 0 ? (
                  <p className="rounded-xl border border-border px-3 py-3 text-sm text-muted-foreground">
                    Tidak ada rule pre-check yang aktif pada kombinasi ini. Ini bukan pernyataan
                    formula aman.
                  </p>
                ) : (
              ...
              <Card>
                <CardTitle
                  ...
                  sub="Status deteksi dini risiko untuk formula ini"
                />
                ...
                <div className="border-l-2 border-l-warning bg-warning-soft/40 p-3">
                  ...
                  <p className="mt-1.5 text-xs leading-relaxed text-muted-foreground">
                    ...
                    dikonfirmasi peneliti. Draf formula saja tidak cukup, jadi tidak ada forecast
                    risiko yang diterbitkan di layar ini.
                  </p>
            ...
            <Card>
                ...
                right={
                  <span className="flex items-center gap-2">
                    <select
                      ...
                      value={paramBaru}
                      onChange={(e) => setParamBaru(e.target.value)}
                    >
                    ...
                    <div key={t.id} className="rounded-xl border border-border p-3">
                      ...
                      <div className="mt-2 flex items-center gap-2">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setTargets((prev) =>
                              prev.map((x, i) =>
                                i === idx ? { ...x, toleransi: Number(e.target.value) } : x,
                              ),
                            )
                          }
                        />
                      ...
                      {p && (
                        <p className="mt-2 text-xs">
                          ...
                          <span
                            className={
                              p.lolos ? "font-semibold text-success" : "font-semibold text-warning"
                            }
                          >
                            {p.prediksi} {p.unit}
                          </span>
                          <span className="text-muted-foreground">
                            {" "}
                            · heuristik penyusunan target, bukan keluaran F3
                          </span>
                        </p>
                      )}
              ...
              {langkah === 2 && (
                <div className="mt-4 border-t border-border pt-4">
                  <PrimaryButton
                    onClick={simpanParameter}
                    disabled={targets.length === 0 || menganalisis}
                  >
                    {menganalisis ? (
                    ...
                    {menganalisis
                      ? "AI menganalisis formula dan parameter"
                      : "Simpan parameter dan jalankan analisis"}
                  </PrimaryButton>
                  {menganalisis && (
          ...
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            {langkah === 1 ? (
              <PrimaryButton
                ...
                disabled={Math.abs(totalPersen - 100) >= 0.5}
              >
                <Check className="size-4" /> Simpan formula
              </PrimaryButton>
            ) : langkah === 2 ? (
              <span className="text-xs text-muted-foreground">
                Simpan parameter untuk membuka prediktif pre-check kompatibilitas.
              </span>
            ) : (
          ...
          {langkah === 3 && (
            <div className="space-y-5">
              <Card>
                <CardTitle
                  title="Pratinjau jurnal praktikum kosong"
                  sub="AI menyiapkan kerangka, peneliti tetap wajib menguji ulang formula di laboratorium."
                />
              ...
              <Card>
                <CardTitle
                  ...
                  sub="Tata letak mengikuti format jurnal praktikum dengan ruang kerja R&D kosmetik yang lebih lengkap."
                />
                <PratinjauDokumen brief={brief} bahan={bahan} targets={targets} />
                <div className="mt-4 flex flex-wrap items-center justify-between gap-3 border-t border-border pt-4">
                  <p className="flex items-center gap-2 text-xs text-muted-foreground">
                    <FileText className="size-4 text-brand" /> Editor teks tersedia setelah jurnal
                    disimpan.
                  </p>
...
function PratinjauDokumen({
  ...
  return (
    <div className="mx-auto max-h-[620px] max-w-[720px] overflow-hidden border border-border bg-card px-8 py-10 shadow-sm sm:px-14">
      <img
        src={logoDark}
        alt="paralab.ai, Electronic Lab Notebook Vinara R&D"
        className="mb-5 h-12 w-auto max-w-[210px] object-contain object-left"
      />
      <p className="text-right text-xs leading-5 text-foreground">
        Tanggal praktikum:{" "}
        {new Date().toLocaleDateString("id-ID", { day: "2-digit", month: "long", year: "numeric" })}
        <br />
        Peneliti: Peneliti aktif
      </p>
      <div className="my-7 text-center">
        <p className="text-xs font-bold uppercase text-muted-foreground">
          Modul R&D Formulasi Kosmetik
        </p>
        <h3 className="mt-1 text-base font-bold uppercase text-foreground">
          {brief.judul || "Judul penelitian"}
        </h3>
      </div>
      <div className="space-y-5 text-xs leading-5 text-foreground">
        <section className="grid grid-cols-[2.25rem_1fr] gap-2">
          <strong>I.</strong>
          <div>
            <strong>TUJUAN PERCOBAAN</strong>
        ...
        <section className="grid grid-cols-[2.25rem_1fr] gap-2">
          <strong>II.</strong>
          <div>
            <strong>TEORI DASAR DAN HIPOTESIS</strong>
            <p className="mt-1">{batch.hipotesis}</p>
          </div>
        </section>
        <section className="grid grid-cols-[2.25rem_1fr] gap-2">
          <strong>III.</strong>
          <div>
            ...
            <p className="mt-1">
              Formula terdiri dari {bahan.length} bahan dan {targets.length} parameter mutu kritis.
            </p>
          </div>
        </section>
        <section className="grid grid-cols-[2.25rem_1fr] gap-2">
          <strong>IV.</strong>
          <div>
            <strong>DIAGRAM ALIR PERCOBAAN DAN DATA PENGAMATAN</strong>
            <div className="mt-2 grid h-28 grid-cols-2 border border-border">
              <div className="border-r border-border p-2">Diagram alir proses</div>
              <div className="p-2">Data sensor dan observasi</div>
            </div>
          </div>
        </section>
        <p className="text-center text-[10px] text-muted-foreground">
          Pratinjau dipotong. Dokumen lengkap memuat formula, data kimia bahan, mutu, stabilitas,
          mikrobiologi, pembahasan, dan pengesahan.
        </p>
      </div>
    </div>
  );
    ...
    <div className="space-y-4 text-sm">
      ...
      <div>
        ...
        <p className="mt-1.5 text-muted-foreground">
          {targets.length} baris kosong menunggu pembacaan sensor:{" "}
