...
export function jadwalFotoDroplet(batch: Batch, sekarang: number): TitikFoto[] {
  ...
  for (let hari = 0; hari <= 21; hari += 3) {
    ...
    const status: TitikFoto["status"] = selisih >= 1 ? "selesai" : selisih >= 0 ? "hari ini" : "menunggu";
    titik.push({
      hari,
      tanggal: new Date(waktu).toLocaleDateString("id-ID", { day: "2-digit", month: "short", year: "numeric" }),
      status,
      instruksi:
        hari === 0
...
export function statusPemantauan(proyek: Project, batch: Batch, sekarang: number): StatusPemantauan {
  ...
  const tugas: string[] = [];
  if (kosong.length > 0) tugas.push("Lengkapi " + kosong.length + " parameter uji yang masih kosong: " + kosong.slice(0, 4).map((t) => t.label).join(", "));
  if (!batch.observasi.trim()) tugas.push("Tuliskan observasi proses batch " + batch.nomor);
  if (!batch.feedback.trim()) tugas.push("Isi umpan balik peneliti sebelum menutup batch " + batch.nomor);
  if (batch.status === "pemantauan" && fotoTertunda.length > 0) tugas.push("Unggah foto droplet hari ke " + fotoTertunda.map((t) => t.hari).join(", ") + " pada proses uji coba sampel");
  if (!batch.evaluasi) tugas.push("Tekan selesaikan praktikum batch " + batch.nomor + " untuk mendapat evaluasi");
  if (batch.evaluasi && skor < 70) tugas.push("Skor kesesuaian baru " + skor + " persen, buat batch perbaikan berikutnya");
  if (batch.evaluasi && skor >= 70 && !proyek.dikirimKe) tugas.push("Susun laporan praktikum dan brief risiko naik skala, lalu kirim ke divisi terkait");
  ...
  const pesan = terhenti
    ? "Penelitian ini tidak diperbarui selama " + hariDiam + " hari. Selesaikan pekerjaan yang tertunda agar batch tidak kedaluwarsa."
